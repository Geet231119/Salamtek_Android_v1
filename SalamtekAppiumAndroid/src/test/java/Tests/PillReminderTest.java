...
import Pages.PillReminderPage;
import Utils.ExcelUtils;
import Utils.ExtentTestManager;

public class PillReminderTest extends BaseTest {

	LoginPage loginPage = new LoginPage(getDriver());
	HomePage homePage = new HomePage(getDriver());
	AccountPage accountPage = new AccountPage(getDriver());
	PillReminderPage pillReminderPage = new PillReminderPage(getDriver());
	
	ExcelUtils excelUtils = new ExcelUtils();
	String sheetName = "PillReminder";
	...
	@Test(description = "Verify whether pill reminder works as expected", dataProvider = "dataForPillReminder")
	public void PillReminder(String testCaseID, String testCaseName, String description,
			String executionStatus, String username, String password, String operationName, String pillReminderName, String reminderFor, String repeatAlarmValue) throws Exception {
		...
		testResult = accountPage.selectPillReminderTab();
		if (testResult.equals(false)) {
			excelUtils.updateResult(false);
			Assert.assertEquals(testResult, true);
		}
		testResult = pillReminderPage.verifyPillReminder(pillReminderName, reminderFor, repeatAlarmValue, sheetName, testCaseID);
		if (testResult.equals(false)) {
			excelUtils.updateResult(false);
			Assert.assertEquals(testResult, true);
		}
		testResult = pillReminderPage.openNotification_pill(sheetName, testCaseID);
		if (testResult.equals(false)) {
			excelUtils.updateResult(false);
			Assert.assertEquals(testResult, true);
		}
	...
	@DataProvider(name = "dataForPillReminder")
